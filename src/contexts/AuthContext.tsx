...
export function AuthProvider({ children }) {
  ...
  async function signIn({ email, password }: SignInData) {
    ...
    if (!user) {
      console.log('Falha na autenticação, e-mail ou senha inválida.')
  ...
  async function generateToken(user: UserType, password: string) {
    const base64Password = Buffer.from(password, 'base64');
    
    if (base64Password.toString('utf-8') === user.password) {
      const payload = {
        role: user.role,
        user_id: user.id
      };
